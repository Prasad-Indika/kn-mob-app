...
export default function OrdersListToday({search}) {
  ...
  const ordersData = useSelector((state)=>state.orderSlice.order)

  const setAllOrders = ()=>{
    if(search === 'new'){
      const odrs = ordersData?.data.filter(order => order.status === 'pending');
      setOrders(odrs);
      setOrdersSerch(odrs);
    }else if(search === 'ongoing'){
      const odrs = ordersData?.data.filter(order => order.status === 'assign');
      setOrders(odrs);
      setOrdersSerch(odrs);
    }else if(search === 'complete'){
      const odrs = ordersData?.data.filter(order => order.status === 'complete');
      setOrders(odrs);
      setOrdersSerch(odrs);
    }else{
      const odrs = ordersData?.data
      setOrders(odrs);
      setOrdersSerch(odrs);
    }
  }
...
},[ordersData.data,ordersData.isSuccess])
