...
export default function PaymentDetailsListofPartner({search,onViewClick,partnerId}) {
  ...
  const paymentData = useSelector((state)=>state.paymentByEmployeeSlice.payment)

  const getAllOrders = ()=>{

    if(search === 'ongoing'){
        const odrs = paymentData.data.filter(item => item.order.status === 'assign');
        setOrders(odrs);
      }else if(search === 'complete'){
        const odrs = paymentData.data.filter(item => item.order.status === 'complete');
        setOrders(odrs);
      }else{
        setOrders(paymentData.data);
      }
      
  }

  useEffect(()=>{
    dispatch(getPaymentDetailsByEmployee(partnerId))
  },[])
  ...
  },[paymentData.data,paymentData.isSuccess])
